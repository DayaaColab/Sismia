...
# Nuevos datos
new_data = [[-1.726836,2023], [-3.169910,2023]]
...
sns.regplot(x=X_test['Año'], y=y_test, color='red', scatter_kws={'s': 8})
#sns.regplot(x=X_test['RMS'], y=y_test, color='violet', scatter_kws={'s': 10})
plt.legend(labels=['Latitud(deg)', 'Año'])
...
features = df[['Magnitud Principal','Año']][:subset_size].values
classes = df['Tipo de Magnitud'][:subset_size].values
...
svm2.fit(features, (classes=='MW').astype(int))
